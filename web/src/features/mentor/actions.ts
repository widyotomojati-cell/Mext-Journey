...
Jawaban Dio: ${answer}`,text:{format:{type:"json_schema",name:"mentor_review",strict:true,schema}}})});
  if(!response.ok) {
    if(response.status===401) return {status:"error",message:"API key OpenAI belum valid. Cek OPENAI_API_KEY di Vercel lalu redeploy."};
    if(response.status===429) return {status:"error",message:"Akun API OpenAI sedang kena limit atau belum punya kredit billing. Cek Usage & Billing di platform.openai.com."};
    return {status:"error",message:"Mentor sedang tidak bisa dihubungi. Coba lagi sebentar."};
  }
